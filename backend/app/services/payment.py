...
class PaymentService:
    ...
    async def create_payment_order(self, current_user: User, data: PaymentOrderCreate) -> PaymentResponse:
        ...
        # The doctor fee is authoritative; the browser never controls the charge amount.
        ...
        payment_tx = await self.payment_repo.create({
        ...
        response = PaymentResponse.model_validate(refetched)
        response.razorpay_key_id = settings.RAZORPAY_KEY_ID
        return response

    async def verify_payment(self, current_user: User, data: PaymentVerifyRequest) -> PaymentResponse:
        ...
        if float(appointment.doctor.consultation_fee) != float(tx.amount):
            raise BadRequestException("Consultation fee changed; a new payment order is required.")
